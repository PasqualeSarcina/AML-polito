...
@torch.no_grad()
def match_wsa_nearest_patch_masked(
    src_feat: torch.Tensor,        # [1,C,Hf,Wf]
    trg_feat: torch.Tensor,        # [1,C,Hf,Wf]
    src_kps_xy: torch.Tensor,      # [K,2] pixel coords in [0,out_size)
    out_size: int,
    ...
    *,
    ...
    device = src_feat.device
    src_kps_xy = src_kps_xy.to(device=device, dtype=torch.float32)

    B, C, Hf, Wf = src_feat.shape
    assert B == 1, "This function expects features for a single pair (B=1)."
    assert trg_feat.shape[-2:] == (Hf, Wf), "Expected same grid size for src/trg features."
    ...
    K = src_kps_xy.shape[0]
    pred = torch.full((K, 2), -1.0, device=device, dtype=torch.float32)
    ...
    # Assign keypoint to the NEAREST PATCH CENTER (same as Task1/2)
    ix = torch.clamp(((xs / patch) - 0.5).round().long(), 0, Wf - 1)
    iy = torch.clamp(((ys / patch) - 0.5).round().long(), 0, Hf - 1)
    ...
    # Gather source descriptors: [Kkeep,C]
    src_map = src_feat.squeeze(0)  # [C,Hf,Wf]
    desc = src_map[:, iy2, ix2].transpose(0, 1).contiguous()  # [Kkeep,C]
    desc = F.normalize(desc, p=2, dim=1)
    ...
    # Similarity: [Kkeep, HW] -> mask padded target patches
    sim = (desc @ trg_flat).float()  # float32 for stable softmax/masking
    sim = sim.masked_fill(~trg_valid_flat.unsqueeze(0), -1e9)

    half = wsa_window // 2
    t = max(float(wsa_temp), 1e-8)

    # Predict per kp
    pred_keep = torch.empty((sim.shape[0], 2), device=device, dtype=torch.float32)

    for k in range(sim.shape[0]):
        sim_2d = sim[k].view(Hf, Wf)  # [Hf,Wf]

        # peak
        idx = int(sim_2d.reshape(-1).argmax().item())
        py = idx // Wf
        px = idx - py * Wf

        # window bounds
        y0 = max(py - half, 0); y1 = min(py + half + 1, Hf)
        x0 = max(px - half, 0); x1 = min(px + half + 1, Wf)

        win = sim_2d[y0:y1, x0:x1]  # [h,w]
        wts = F.softmax((win / t).reshape(-1), dim=0).view_as(win)
        ...
        # patch coords (float)
        y_f = torch.sum(yy * wts)
        x_f = torch.sum(xx * wts)

        # patch center -> pixel coords
        pred_keep[k, 0] = (x_f + 0.5) * patch
        pred_keep[k, 1] = (y_f + 0.5) * patch
