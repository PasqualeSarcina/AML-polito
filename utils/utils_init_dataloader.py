...
def init_dataloader(dataset_name: Literal['spair-71k', 'pf-pascal', 'pf-willow', 'ap-10k'], base_dir,
    ...
    match dataset_name:
        case 'spair-71k':
            ...
            dataset = SPairDataset(dataset_size="large", datatype=datatype, transform=transform, base_dir=base_dir)
        case 'pf-pascal':
            from data.pfpascal import PFPascalDataset
            dataset = PFPascalDataset(datatype=datatype, transform=transform, base_dir=base_dir)
        case 'pf-willow':
            from data.pfwillow import PFWillowDataset
            dataset = PFWillowDataset(transform=transform, base_dir=base_dir)
        case 'ap-10k':
            from data.ap10k import AP10KDataset
            dataset = AP10KDataset(datatype=datatype, transform=transform, base_dir=base_dir)
        case _:
