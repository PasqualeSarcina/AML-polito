    ...
    def evaluate(self) -> list[CorrespondenceResult]:
        ...
        with torch.no_grad():
            for batch in tqdm(self.dataloader, total=len(self.dataloader),
                ...
                # features: [1,C,48,48]
                src_ft = self.compute_features(batch["src_img"], batch["src_imname"], category, up_ft_index=[0, 1, 2], t=self.timestep)
                trg_ft = self.compute_features(batch["trg_img"], batch["trg_imname"], category, up_ft_index=[0, 1, 2], t=self.timestep)

                src_ft, trg_ft, dims_used = self._compute_pca(src_ft, trg_ft)
